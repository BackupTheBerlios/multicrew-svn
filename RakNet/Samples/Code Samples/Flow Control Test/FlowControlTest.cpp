...
int main(void)
{
	...
	int time;
	Packet *p;
	unsigned long lastPacketReceipt, lastNotification, lastSend;
	...
	sendinterval=128;
	packetSize=64;
	lastPacketReceipt=lastNotification=RakNetGetTime();
	lastSend=0;
	...
	while (1)
	{
		...
		if (p)
		{
			lastPacketReceipt=RakNetGetTime();
			...
			(rakClient && rakClient->IsConnected()))
		{
			...
			if (lastPacketReceipt + 500 < (unsigned long)time && lastNotification + 500 < (unsigned long)time)
			{

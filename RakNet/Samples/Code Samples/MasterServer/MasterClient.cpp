...
bool MasterClient::Connect(char* host, int masterServerPort, int masterClientPort, int gameServerPort)
{
	...
	localServer.connectionIdentifier.port=gameServerPort;
...
void MasterClient::ConnectionAttemptNotification(char *serverIP, unsigned short serverPort)
{
	...
	stringCompressor->EncodeString(serverIP, 22, &bitStream);
...
{
#ifdef _SHOW_MASTER_SERVER_PRINTF
	printf("Master server indicates a connection request from %s:%i.\n", clientIP, clientPort);
#endif

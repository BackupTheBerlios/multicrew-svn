...
public:
	...
	bool Connect(char* host, int masterServerPort, int masterClientPort, int gameServerPort);
	...
	// This will tell the master server that we are trying to connect to the indicated server.
	// The indicated server will then also try to connect to us, bypassing most NATs
	// And increasing the chance of a successful connection.  You should try connecting to the server

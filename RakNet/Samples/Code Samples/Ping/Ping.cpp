	...
	while (1)
	{
		...
		switch (p->data[0])
		{
			...
			case ID_PONG:
				...
				memcpy((char*)&time, p->data+sizeof(unsigned char), sizeof(unsigned long));
				dataLength = p->length - sizeof(unsigned char) - sizeof(unsigned long);
				printf("ID_PONG from PlayerID:%u:%u.\nPing is %i\nData is %i bytes long.\n",p->playerId.binaryAddress, p->playerId.port, time, dataLength);
				if (dataLength > 0)
					printf("Data is %s\n", p->data+sizeof(unsigned char)+sizeof(unsigned long));
				break;

...
int main(void)
{
	...
	unsigned long packetNumber[32], receivedPacketNumber, receivedTime;
	char str[256];
	char ip[32];
	unsigned long sendInterval, nextSend, currentTime, quitTime;
	...
	if (str[0]=='s' || str[0]=='S')
	{
		...
		printf("(S)ingle threaded or (m)ultithreaded?\n");
		gets(str);
		if (str[0]=='s' || str[0]=='S')
			singleThreaded=true;
		else
		...
		printf("Connecting...\n");
		if (singleThreaded)
			sender->Initialize(1, localPort, -1);
		else
	...
	else
	{
		...
		if (singleThreaded)
			receiver->Initialize(32, localPort, -1);
		else
			...
			while (packet)
			{
				...
				case ID_RESERVED9+1:
					...
					if (receivedPacketNumber!=packetNumber[streamNumber])
						...
						printf("Got %i (channel %i). ", packetNumber[streamNumber], streamNumber);

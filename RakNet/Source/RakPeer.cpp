...
void RakPeer::ResetRemoteSystemData( RemoteSystemStruct *remoteSystem, bool weInitiatedTheConnection )
{
	...
	remoteSystem->staticData.Reset();
	remoteSystem->connectionTime = RakNet::GetTime();
	...
	else
	{
		if ( ( unsigned char ) ( data ) [ 0 ] == ID_CONNECTION_REQUEST )
		{
				...
				length == 1 + sizeof( big::u32 ) + sizeof( RSA_BIT_SIZE ) + 20 )
			{
				...
				if ( ( unsigned char ) ( data ) [ 0 ] == ID_SECURED_CONNECTION_CONFIRMATION &&
					length == 1 + 20 + sizeof( RSA_BIT_SIZE ) )
				{
					...
					if ( memcmp( sha1.GetHash(), data + 1, 20 ) == 0 )
					{
						...
						if ( rakPeer->randomNumberExpirationTime < RakNet::GetTime() )
						{
							sha1.Reset();
							sha1.Update( ( unsigned char* ) & playerId.binaryAddress, sizeof( playerId.binaryAddress ) );
							sha1.Update( ( unsigned char* ) & playerId.port, sizeof( playerId.port ) );
					...
					if ( ( unsigned char ) ( data ) [ 0 ] == ID_CONNECTION_REQUEST_ACCEPTED && length == sizeof( ConnectionAcceptStruct ) )
					{
						...
						if ( ( ( unsigned char ) ( data ) [ 0 ] == ID_PING || ( unsigned char ) ( data ) [ 0 ] == ID_PING_OPEN_CONNECTIONS )
							&& length == sizeof( UnconnectedPingStruct ) )
						{
							...
							if ( ( unsigned char ) ( data ) [ 0 ] == ID_ADVERTISE_SYSTEM && length == sizeof( unsigned char ) )
							{
								...
								if ( ( unsigned char ) ( data ) [ 0 ] == ID_PONG && length >= sizeof( UnconnectedPingStruct ) )
								{
									Packet * packet = PacketPool::Instance() ->GetPointer();
									unsigned long time;
									...
									if ( ( unsigned char ) ( data ) [ 0 ] == ID_NO_FREE_INCOMING_CONNECTIONS && length == sizeof( unsigned char ) )
									{
										Packet * packet = PacketPool::Instance() ->GetPointer();
										packet->data = new unsigned char [ 1 * sizeof( unsigned char ) ];
										...
										if ( ( unsigned char ) ( data ) [ 0 ] == ID_CONNECTION_BANNED && length == sizeof( unsigned char ) )
										{
											Packet * packet = PacketPool::Instance() ->GetPointer();
											packet->data = new unsigned char [ 1 * sizeof( unsigned char ) ];
	...
	if ( requestedConnectionsList.size() > 0 )
	{
		...
		while ( remoteSystemIndex < requestedConnectionsList.size() )
		{
			...
			if ( time - requestedConnectionsList[ remoteSystemIndex ] ->time > SYN_COOKIE_OLD_RANDOM_NUMBER_DURATION * 2 )
			{
				...
				if ( time > requestedConnectionsList[ remoteSystemIndex ] ->nextRequestTime )
				{

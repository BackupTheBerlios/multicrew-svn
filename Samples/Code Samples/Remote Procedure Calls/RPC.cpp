...
{
	// We send the data as a bitstream.  However, we know the first 4 bytes is an unsigned long
	// That holds a timestamp.  So just take a shortcut and memcpy over
	unsigned long timestamp;
	memcpy((char*)&timestamp, input, sizeof(unsigned long));

	printf("In clientRPC. Time sent: %i. Message = %s\n", timestamp, input+sizeof(unsigned long));
}
...
int main(void)
{
	...
	state=0;
	nextActionTime=RakNetGetTime()+1000;
	...
	while (state<4)
	{
		...
		if (RakNetGetTime() > nextActionTime)
		{
			...
			if (state==1)
			{
				...
				rakClient->RPC("serverRPC",str, (long)((strlen(str)+1)*8), HIGH_PRIORITY, RELIABLE, 0, false);
			}
			else if (state==2)
			{
				...
				BitStream bitStream;
				bitStream.Write(RakNetGetTime()); // Write the system time to the first 4 bytes

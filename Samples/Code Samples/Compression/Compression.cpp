...
class ServerMultiplayer : public Multiplayer<InterfaceType>
{
	void ProcessUnhandledPacket(Packet *packet, unsigned char packetIdentifier, InterfaceType *interfaceType)
	{
		...
		interfaceType->Send(message, (const long)strlen(message)+1, HIGH_PRIORITY, RELIABLE_ORDERED, 0, packet->playerId, true);
	}
};
...
int main(void)
{
	...
	{
		unsigned long frequencyData[256];
		int numRead;
		numRead=(int)fread(frequencyData, sizeof(unsigned long), 256, serverToClientFrequencyTableFilePointer);
        if (numRead != 256)
	...
	else
	{
		unsigned long frequencyData[256];
		int numRead;
		numRead=(int)fread(frequencyData, sizeof(unsigned long), 256, clientToServerFrequencyTableFilePointer);
		if (numRead != 256)
			puts("Error reading data");
		else
		{
	...
	while (1)
	{
		if (kbhit())
		{
			...
			if (isServer)
			{
				...
				rakServer->Send(message2, (const long)strlen(message2)+1, HIGH_PRIORITY, RELIABLE_ORDERED, 0, UNASSIGNED_PLAYER_ID, true);
			}
			else
			{
				...
				// HIGH_PRIORITY doesn't actually matter here because we don't use any other priority
				// RELIABLE_ORDERED means make sure the message arrives in the right order
				rakClient->Send(message, (const long)strlen(message)+1, HIGH_PRIORITY, RELIABLE_ORDERED, 0);
			}
		}
	...
	else
	{
		...
		fwrite(frequencyData, sizeof(unsigned long), 256, serverToClientFrequencyTableFilePointer);
		fclose(serverToClientFrequencyTableFilePointer);
	}

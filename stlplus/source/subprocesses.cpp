...
////////////////////////////////////////////////////////////////////////////////
// environment-vector
// Windoze environment is a single string containing null-terminated name=value strings and the whole terminated by a null
                       ...
                       bool connect_stdin, bool connect_stdout, bool connect_stderr)
{
  ...
  IF_DEBUG(ferr << "stdin_pipe, parent = " << child_in << " => child input " << parent_stdin << endl);
  ...
  IF_DEBUG(ferr << "stdout_pipe = child output " << parent_stdout << " => parent " << child_out << endl);
  ...
  IF_DEBUG(ferr << "stderr_pipe = child error " << parent_stderr << " => parent " << child_err << endl);
                       ...
                       bool connect_stdin, bool connect_stdout, bool connect_stderr)
{
  ...
  if (connect_stdin)
    pipe(stdin_pipe);
  IF_DEBUG(ferr << "stdin_pipe = [" << stdin_pipe[0] << "," << stdin_pipe[1] << "]\n");

  int stdout_pipe [2] = {-1, -1};
  if (connect_stdout)
    pipe(stdout_pipe);
  IF_DEBUG(ferr << "stdout_pipe = [" << stdout_pipe[0] << "," << stdout_pipe[1] << "]\n");

  int stderr_pipe [2] = {-1, -1};
  if (connect_stderr)
    pipe(stderr_pipe);
  IF_DEBUG(ferr << "stderr_pipe = [" << stderr_pipe[0] << "," << stderr_pipe[1] << "]\n");
  ...
  case 0:  // in child;
  {
    ...
    IF_DEBUG(ferr << "child started OK" << endl);
    if (connect_stdin)
    {
      ::close(stdin_pipe[1]);
    ...
    IF_DEBUG(ferr << "exec returned, exiting with status = " << errno << endl);
    exit(errno);
  }
  break;
  default:  // in parent
  {
    ...
    {
      int wait_ret_val = waitpid(pid, &wait_status, 0);
      IF_DEBUG(ferr << "waitpid returned value = " << wait_ret_val << ", errno = " << errno << ", wait status = " << wait_status << endl);
      if (wait_ret_val != -1 || errno != EINTR) break;
    }
    // establish whether an error occurred
    if (WIFSIGNALED(wait_status))
    {
      IF_DEBUG(ferr << "child stopped by signal " << WTERMSIG(wait_status) << endl);
      // set_error(errno);
    ...
    else if (WIFEXITED(wait_status))
    {
      ...
      IF_DEBUG(ferr << "child exited with status " << status << endl);
      if (status != 0)
        result = false;
    }
    pid = -1;
...
bool backtick_subprocess::callback(void)
{
  ...
  IF_DEBUG(ferr << "backtick ended with " << error_text() << endl);
  return !error();
}
...
void async_subprocess::set_error(int e)
{
  DEBUG_TRACE;
  err = e;
  IF_DEBUG(ferr << "error flag set to " << err << " = " << error_text() << endl);
}
                             ...
                             bool connect_stdin, bool connect_stdout, bool connect_stderr)
{
  ...
  default:  // in parent
  {
    ...
    if (connect_stdin)
    {
      ...
      {
        IF_DEBUG(ferr << "fcntl returned an error for stdin" << endl);
        set_error(errno);
        result = false;
      }
    }
    if (connect_stdout)
    {
      ...
      {
        IF_DEBUG(ferr << "fcntl returned an error for stdout" << endl);
        set_error(errno);
        result = false;
      }
    }
    if (connect_stderr)
    {
      ...
      {
        IF_DEBUG(ferr << "fcntl returned an error for stderr" << endl);
        set_error(errno);
        result = false;
      }
    }
...
//  static otext& operator << (otext& str, const fd_set& set)
//  {
//    long long value = (((long long)set.fds_bits[1]) << 32) | ((long long)set.fds_bits[0]);
//    str << to_string(value,2,radix_none);
//    return str;
//  }

//  int async_subprocess::write_stdin (string& buffer)
//  {
//    DEBUG_TRACE;
//    if (child_in == -1) return -1;
//    timeval delay = {0, 0};
//    fd_set inputs;
//    FD_ZERO(&inputs);
//    FD_SET(child_in, &inputs);
//    IF_DEBUG(ferr << "calling select(" << child_in+1 << "," << inputs << ")" << endl);
//    // Note: under Cygwin, select never returns zero, in other words the pipe is always available to write and the write then blocks
//    //       I hope this works under true Unices
//    int result = select(child_in+1, 0, &inputs, 0, &delay);
//    IF_DEBUG(ferr << "select(" << child_in+1 << "," << inputs << ") returned " << result << endl);
//    if (result == -1)
//    {
//      // error on select - close the pipe and return the error condition
//      set_error(errno);
//      close_stdin();
//      return -1;
//    }
//    else if (result == 0)
//    {
//      // not ready to write, so simply return with 0 bytes written
//      return 0;
//    }
//    int bytes = write(child_in, buffer.c_str(), buffer.size());
//    IF_DEBUG(ferr << "wrote " << bytes << " bytes" << endl);
//    if (bytes == -1)
//    {
...
int async_subprocess::write_stdin (std::string& buffer)
{
  ...
  {
    // not ready
    IF_DEBUG(ferr << "write not ready" << endl);
    return 0;
  }
  if (bytes == -1)
  {
    // error on write - close the pipe and give up
    IF_DEBUG(ferr << "write error " << errno << endl);
    set_error(errno);
    close_stdin();
    return -1;
  }
...
int async_subprocess::read_stdout (std::string& buffer)
{
  ...
  {
    IF_DEBUG(ferr << "Peek failed" << endl);
    if (GetLastError() != ERROR_BROKEN_PIPE)
      set_error(GetLastError());
    close_stdout();
    return -1;
  }
  IF_DEBUG(ferr << "there are " << buffer_size << " bytes to read" << endl);
  if (buffer_size == 0) return 0;
  DWORD bytes = 0;
  char* tmp = new char[buffer_size];
  if (!ReadFile(child_out, tmp, buffer_size, &bytes, 0))
  {
    IF_DEBUG(ferr << "Read failed" << endl);
    set_error(GetLastError());
    close_stdout();
    delete[] tmp;
    return -1;
...
//  int async_subprocess::read_stdout (string& buffer)
//  {
//    DEBUG_TRACE;
//    if (child_out == -1) return -1;
//    timeval delay = {0, 0};
//    fd_set outputs;
//    FD_ZERO(&outputs);
//    FD_SET(child_out, &outputs);
//    IF_DEBUG(ferr << "calling select(" << child_out+1 << "," << outputs << ")" << endl);
//    int result = select(child_out+1, &outputs, 0, 0, &delay);
//    IF_DEBUG(ferr << "select(" << child_out+1 << "," << outputs << ") returned " << result << endl);
//    if (result == -1)
//    {
//      set_error(errno);
//      return -1;
...
//    char* tmp = new char[buffer_size];
//    int bytes = read(child_out, tmp, buffer_size);
//    IF_DEBUG(ferr << "read " << bytes << " bytes" << endl);
//    if (bytes == 0)
//    {
//      close_stdout();
//    else if (bytes == -1)
...
int async_subprocess::read_stdout (std::string& buffer)
{
  ...
  {
    // not ready
    IF_DEBUG(ferr << "read not ready" << endl);
    delete[] tmp;
    return 0;
  }
  if (bytes == -1)
  {
    // error
    IF_DEBUG(ferr << "read error " << errno << endl);
    set_error(errno);
    close_stdout();
    delete[] tmp;
    return -1;

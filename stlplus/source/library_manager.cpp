...
static lm_dependencies& out_of_date_check (std::map<std::pair<std::string,lm_unit_name>,lm_dependencies>& visited,
                                                    const library_manager* manager, const std::string& library, const lm_unit_name& name)
{
  DEBUG_TRACE;
  IF_DEBUG(ferr << "checking unit " << library << ":" << name << endl);
  // the visited field contains an entry if a unit has been visited - it also contains the reason for out-of-date-ness
  // this is initially set empty (up to date) since the unit has to be added before it is checked just in case there is a recursive loop
  ...
  if (!unit)
  {
    ...
    IF_DEBUG(ferr << "unit is missing" << endl);
  ...
  else
  {
    ...
    IF_DEBUG(ferr << "unit modified at " << unit_modified << endl);
    ...
    if (unit->source_file_present())
    {
      ...
      IF_DEBUG(ferr << "source " << source_file << " modified at " << source_modified << endl);  
      if (source_modified == 0 || source_modified > unit_modified)
      {
        IF_DEBUG(ferr << "OUT OF DATE: source " << source_file << " is more recent" << endl);
    ...
    for (unsigned i = 0; i < unit->file_size(); i++)
    {
      ...
      IF_DEBUG(ferr << "file dependency on " << file_full << " modified at " << source_modified << endl);
      if (source_modified == 0 || source_modified > unit_modified)
      {
        IF_DEBUG(ferr << "OUT OF DATE: dependency file " << file_full << " is more recent" << endl);
    ...
    for (unsigned j = 0; j < unit->unit_size(); j++)
    {
      ...
      IF_DEBUG(ferr << "found dependency " << dependency << endl);
      ...
      if (!other_unit)
      {
        IF_DEBUG(ferr << "OUT OF DATE: dependent unit " << dependency << " not found" << endl);
        visited[full_name].unit_add(dependency);
      }
      else
      {
        time_t other_modified = other_unit->modified();
        IF_DEBUG(ferr << "unit dependency on " << dependency << " modified at " << other_modified << endl);
        if (other_modified == 0 || other_modified > unit_modified)
        {
          IF_DEBUG(ferr << "OUT _OF DATE: unit " << dependency << " is more recent" << endl);
          visited[full_name].unit_add(dependency);
        }
        else
        {
          ...
          {
            IF_DEBUG(ferr << "unit " << dependency << " has already been visited" << endl);
            ...
            {
              IF_DEBUG(ferr << "OUT OF DATE: unit " << dependency << " was out of date on last visit" << endl);
              visited[full_name].unit_add(dependency);
            }
          }
          else
          {
            IF_DEBUG(ferr << "recursing on unit " << dependency << endl);
            // the unit hasn't been visited before, so recurse on it now
            lm_dependencies other_dependencies = out_of_date_check(visited, manager, other_name.first, other_name.second);
            if (!other_dependencies.empty())
            {
              IF_DEBUG(ferr << "OUT OF DATE: unit " << dependency << " was out of date on first visit" << endl);
              visited[full_name].unit_add(dependency);
            }
          }
        }
  ...
  IF_DEBUG(ferr << "out-of-date-ness for unit " << library << ":" << name << " is:" << endl << visited[full_name]);
...
bool lm_library::erase_by_source(const std::string& source_file)
{
  ...
  IF_DEBUG(ferr << "erasing units from file " << source_file_full << endl);
  ...
  for (std::vector<lm_unit_name>::iterator i = units.begin(); i != units.end(); i++)
  {
    ...
    if (unit && unit->source_file_present())
    {
      ...
      IF_DEBUG(ferr << "trying unit " << unit->unit_name() << " from " << file_full << endl);
      if (file_full == source_file_full)
      {
        IF_DEBUG(ferr << "erasing unit " << *unit << endl);
        erase(*i);
        result = true;
      }
    }
...
{
  DEBUG_TRACE;
  IF_DEBUG(ferr << "adding type " << type << " for " << description << endl);
  bool result = true;
...
{
  DEBUG_TRACE;
  IF_DEBUG(ferr << "removing type " << type << endl);
  bool result = true;
  for (library_list::iterator i = m_libraries.begin(); i != m_libraries.end(); i++)
  {
    DEBUG_ASSERT(i->m_manager == this);
...
{
  DEBUG_TRACE;
  IF_DEBUG(ferr << "mapping file = " << mapping_file << endl);
...
bool library_manager::load_mappings(const std::string& mapping_file)
{
  ...
  {
    IF_DEBUG(ferr << "could not find mapping file = " << mapping_file << endl);
    return false;
  }
  iftext input(mapping_file);
  if (input.error())
  {
    IF_DEBUG(ferr << "error opening mapping file = " << mapping_file << endl);
    return false;
  }
  ...
  for (std::string path = ""; input.getline(path); line++)
  {
    ...
    IF_DEBUG(ferr << "full path = " << full_path << endl);
    if (!library_manager::is_library(full_path))
    {
      IF_DEBUG(ferr << "path " << full_path << " is not a library" << endl);
      result = false;
    }
    else
    {
      ...
      if (!lib)
      {
        IF_DEBUG(ferr << "open of library at " << full_path << " failed" << endl);
        result = false;
      }
      else
      {
...
bool library_manager::set_ini_manager(ini_manager* ini_files, const std::string& library_section, const std::string& work_name)
{
  ...
  IF_DEBUG(ferr << "Ini manager = \n"; ::print(ferr, *ini_files, 1););
  bool result = true;
  ...
  if (m_ini_files->section_exists(m_ini_section))
  {
    ...
    for (unsigned i = 0; i < library_names.size(); i++)
    {
      ...
      {
        IF_DEBUG(ferr << "skipping empty definition for " << library_name << endl);
      }
      else if (library_name == m_ini_work)
      {
        IF_DEBUG(ferr << "work library " << library_name << " = " << m_ini_files->variable_value(m_ini_section, library_name) << endl);
      ...
      else
      {
        ...
        IF_DEBUG(ferr << "library " << library_name << " = " << value << " in " << filename << endl);
...
bool library_manager::save_mappings (void)
{
  ...
  {
    IF_DEBUG(ferr << "saving to mapping file " << m_mapping_file << endl);
    if (m_libraries.size() == 0)
    {
      // if the file would be empty, delete it
      if (!file_delete(m_mapping_file))
      {
        IF_DEBUG(ferr << "failed to delete mapping file " << m_mapping_file << endl);
        result = false;
      }
    }
    else
    {
      ...
      if (output.error())
      {
        IF_DEBUG(ferr << "failed to open " << m_mapping_file << " for writing " << endl);
        result = false;
      }
      else
      {
        ...
        for (unsigned i = 0; i < libraries.size(); ++i)
        {
          if (libraries[i] != work_name())
          {
            ...
            IF_DEBUG(ferr << "path for library " << libraries[i] << " = " << lib_path << endl);
            output << lib_path << endl;
          }
        }
        if (output.error())
        {
          IF_DEBUG(ferr << "failed to save " << m_mapping_file << " for writing " << endl);
          result = false;
        }
      }
    }
  ...
  if (m_ini_files)
  {
    ...
    for (unsigned i = 0; i < ini_names.size(); i++)
    {
      ...
      else
      {
        ...
        IF_DEBUG(ferr << "library " << ini_name << " = " << value << " in " << filename << endl);
        std::string full_ini_path = folder_to_path(folder_part(filename), value);
        std::string full_lib_path = folder_to_path(path(ini_name));
        IF_DEBUG(ferr << "library path = " << full_lib_path << ", mapped path = " << full_ini_path << endl);
        if (full_ini_path != full_lib_path)
        {
          ...
          IF_DEBUG(ferr << "path has changed - adding variable " << ini_name << " = " << relative_path << endl);
    ...
    for (unsigned j = 0; j < lib_names.size(); j++)
    {
      ...
      if (std::find(ini_names.begin(), ini_names.end(), lib_name) == ini_names.end())
      {
        ...
        std::string relative_path = folder_to_relative_path(folder_part(filename), full_lib_path);
        IF_DEBUG(ferr << "path missing - adding variable " << lib_name << " = " << relative_path << endl);
    ...
    if (!m_ini_files->save())
    {
      IF_DEBUG(ferr << "failed to save ini file" << endl);
      result = false;
    }
  }
  return result;
...
{
  DEBUG_TRACE;
  IF_DEBUG(ferr << "name = " << name << " path = " << path << endl);
...
bool library_manager::purge(const std::string& name)
{
  ...
  library_list::iterator found = local_find(name);
  if (found == m_libraries.end()) return false;
  DEBUG_ASSERT(found->m_manager == this);
  return found->purge();
}
...
bool library_manager::load(void)
{
  ...
  for (library_list::iterator i = m_libraries.begin(); i != m_libraries.end(); i++)
  {
    IF_DEBUG(ferr << "name = " << i->name() << endl);
    DEBUG_ASSERT(i->m_manager == this);
    result &= i->load();
  }
  return result;
...
bool library_manager::up_to_date(const std::string& library, const lm_unit_name& name) const
{
  ...
  IF_DEBUG(ferr << "up to date check - result:" << endl; ::print(ferr, reason, 1));
  return reason.empty();
}

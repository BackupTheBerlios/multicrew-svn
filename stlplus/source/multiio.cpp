...
{
  ombuff* buf = dynamic_cast<ombuff*>(m_buffer.pointer());
  DEBUG_ASSERT(buf);
...
{
  const ombuff* buf = dynamic_cast<const ombuff*>(m_buffer.pointer());
  DEBUG_ASSERT(buf);
...
{
  imbuff* buf = dynamic_cast<imbuff*>(m_buffer.pointer());
  DEBUG_ASSERT(buf);
  return buf ? buf->add(str) : (unsigned)-1;
}
...
{
  const imbuff* buf = dynamic_cast<const imbuff*>(m_buffer.pointer());
  DEBUG_ASSERT(buf);
  return buf ? buf->count() : 0;
}

...
{
  return digraph_iterator<NT,AT,NRef,NPtr>::const_iterator(m_owner,m_node);
}

template<typename NT, typename AT, typename NRef, typename NPtr>
typename digraph_iterator<NT,AT,NRef,NPtr>::iterator digraph_iterator<NT,AT,NRef,NPtr>::deconstify (void) const
{
  return digraph_iterator<NT,AT,NRef,NPtr>::iterator(m_owner,m_node);
}
...
{
  return digraph_arc_iterator<NT,AT,NRef,NPtr>::const_iterator(m_owner, m_arc);
}

template<typename NT, typename AT, typename NRef, typename NPtr>
typename digraph_arc_iterator<NT,AT,NRef,NPtr>::iterator digraph_arc_iterator<NT,AT,NRef,NPtr>::deconstify (void) const
{
  return digraph_arc_iterator<NT,AT,NRef,NPtr>::iterator(m_owner, m_arc);
}
...
typename digraph<NT,AT>::const_arc_vector digraph<NT,AT>::constify_arcs(const PARAMETER_TYPENAME digraph<NT,AT>::arc_vector& arcs) const
  throw(wrong_object,null_dereference,end_dereference)
{
  digraph<NT,AT>::const_arc_vector result;
...
typename digraph<NT,AT>::arc_vector digraph<NT,AT>::deconstify_arcs(const PARAMETER_TYPENAME digraph<NT,AT>::const_arc_vector& arcs) const
  throw(wrong_object,null_dereference,end_dereference)
{
  digraph<NT,AT>::arc_vector result;
  for (unsigned i = 0; i < arcs.size(); i++)
  {
    arcs[i].check(this);
...
typename digraph<NT,AT>::const_path_vector digraph<NT,AT>::constify_paths(const PARAMETER_TYPENAME digraph<NT,AT>::path_vector& paths) const
  throw(wrong_object,null_dereference,end_dereference)
{
  digraph<NT,AT>::const_path_vector result;
...
typename digraph<NT,AT>::path_vector digraph<NT,AT>::deconstify_paths(const PARAMETER_TYPENAME digraph<NT,AT>::const_path_vector& paths) const
  throw(wrong_object,null_dereference,end_dereference)
{
  digraph<NT,AT>::path_vector result;
  for (unsigned i = 0; i < paths.size(); i++)
...
typename digraph<NT,AT>::const_node_vector digraph<NT,AT>::constify_nodes(const PARAMETER_TYPENAME digraph<NT,AT>::node_vector& nodes) const
  throw(wrong_object,null_dereference,end_dereference)
{
  digraph<NT,AT>::const_node_vector result;
...
typename digraph<NT,AT>::node_vector digraph<NT,AT>::deconstify_nodes(const PARAMETER_TYPENAME digraph<NT,AT>::const_node_vector& nodes) const
  throw(wrong_object,null_dereference,end_dereference)
{
  digraph<NT,AT>::node_vector result;
  for (unsigned i = 0; i < nodes.size(); i++)
  {
    nodes[i].check(this);
...
digraph<NT,AT>& digraph<NT,AT>::operator=(const digraph<NT,AT>& r)
{
  ...
  std::map<digraph<NT,AT>::const_iterator,digraph<NT,AT>::iterator> xref;
  for (digraph<NT,AT>::const_iterator n = r.begin(); n != r.end(); n++)
    xref[n] = insert(*n);
  // second phase is to copy the arcs, using the map to convert the old to and from nodes to the new nodes
  for (digraph<NT,AT>::const_arc_iterator a = r.arc_begin(); a != r.arc_end(); a++)
...
{
  unsigned count = 0;
  for (digraph<NT,AT>::const_iterator i = begin(); i != end(); i++)
    count++;
  return count;
}
...
typename digraph<NT,AT>::iterator digraph<NT,AT>::insert(const NT& node_data)
{
  ...
  return digraph<NT,AT>::iterator(this,new_node);
}
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  ...
  return digraph<NT,AT>::iterator(this,m_next);
}
...
void digraph<NT,AT>::clear(void)
{
  ...
  for (digraph<NT,AT>::iterator i = begin(); i != end(); )
    i = erase(i);
}
...
{
  return digraph<NT,AT>::const_iterator(this,m_nodes->m_next);
}

template<typename NT, typename AT>
typename digraph<NT,AT>::iterator digraph<NT,AT>::begin(void)
{
  return digraph<NT,AT>::iterator(this,m_nodes->m_next);
}

template<typename NT, typename AT>
typename digraph<NT,AT>::const_iterator digraph<NT,AT>::end(void) const
{
  return digraph<NT,AT>::const_iterator(this,m_nodes);
}

template<typename NT, typename AT>
typename digraph<NT,AT>::iterator digraph<NT,AT>::end(void)
{
  return digraph<NT,AT>::iterator(this,m_nodes);
}
...
{
  iter.check(this);
  return digraph<NT,AT>::const_arc_iterator(this,iter.m_node->m_inputs[i]);
}
...
{
  iter.check(this);
  return digraph<NT,AT>::arc_iterator(this,iter.m_node->m_inputs[i]);
}
...
{
  iter.check(this);
  return digraph<NT,AT>::const_arc_iterator(this,iter.m_node->m_outputs[i]);
}
...
{
  iter.check(this);
  return digraph<NT,AT>::arc_iterator(this,iter.m_node->m_outputs[i]);
}
...
{
  unsigned count = 0;
  for (digraph<NT,AT>::const_arc_iterator i = arc_begin(); i != arc_end(); i++)
    count++;
  return count;
}
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  ...
  return digraph<NT,AT>::arc_iterator(this,new_arc);
}
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  ...
  delete iter.m_arc;
  return digraph<NT,AT>::arc_iterator(this,m_next);
}

template<typename NT, typename AT>
void digraph<NT,AT>::arc_clear(void)
{
  for (digraph<NT,AT>::arc_iterator a = arc_begin(); a != arc_end(); )
    a = arc_erase(a);
}
...
{
  return digraph<NT,AT>::const_arc_iterator(this,m_arcs->m_next);
}

template<typename NT, typename AT>
typename digraph<NT,AT>::arc_iterator digraph<NT,AT>::arc_begin(void)
{
  return digraph<NT,AT>::arc_iterator(this,m_arcs->m_next);
}

template<typename NT, typename AT>
typename digraph<NT,AT>::const_arc_iterator digraph<NT,AT>::arc_end(void) const
{
  return digraph<NT,AT>::const_arc_iterator(this,m_arcs);
}

template<typename NT, typename AT>
typename digraph<NT,AT>::arc_iterator digraph<NT,AT>::arc_end(void)
{
  return digraph<NT,AT>::arc_iterator(this,m_arcs);
}
...
{
  iter.check(this);
  return digraph<NT,AT>::const_iterator(this,iter.m_arc->m_from);
}
...
{
  iter.check(this);
  return digraph<NT,AT>::iterator(this,iter.m_arc->m_from);
}
...
{
  iter.check(this);
  return digraph<NT,AT>::const_iterator(this,iter.m_arc->m_to);
}
...
{
  iter.check(this);
  return digraph<NT,AT>::iterator(this,iter.m_arc->m_to);
}
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  ...
  {
    digraph<NT,AT>::const_iterator from = arc_from(input(to, arc));
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  ...
  for (unsigned arc = 0; arc < fanout(from); arc++)
  {
    digraph<NT,AT>::const_iterator to = arc_to(output(from, arc));
  ...
  digraph<NT,AT>::sort(PARAMETER_TYPENAME digraph<NT,AT>::arc_select_fn select) const
{
  //DEBUG_TRACE;
  digraph<NT,AT>::const_node_vector result;
  digraph<NT,AT>::const_arc_vector errors;
  // build a map containing the number of fanins to each node that must be visited before this one
  std::map<digraph<NT,AT>::const_iterator,unsigned> fanin_map;
  for (digraph<NT,AT>::const_iterator n = begin(); n != end(); n++)
  {
    unsigned predecessors = 0;
    // only count predecessors connected by selected arcs
    for (unsigned f = 0; f < fanin(n); f++)
    {
      digraph<NT,AT>::const_arc_iterator input_arc = input(n,f);
      digraph<NT,AT>::const_iterator predecessor = arc_from(input_arc);
    ...
    {
      result.push_back(n);
      //IF_DEBUG(ferr << "node "; print_node(ferr,n); ferr << " has no predecessors" << endl); 
    }
    else
    {
      fanin_map[n] = predecessors;
      //IF_DEBUG(ferr << "added node "; print_node(ferr,n); ferr << " with " << to_string(predecessors) << " predecessors" << endl); 
    }
  }
  ...
  for(unsigned i = 0; !fanin_map.empty(); )
  {
    ...
    for (; i < result.size(); i++)
    {
      ...
      digraph<NT,AT>::const_iterator current = result[i];
      //IF_DEBUG(ferr << "visiting node "; print_node(ferr, current); ferr << endl); 
      for (unsigned f = 0; f < fanout(current); f++)
      {
        // only consider successors connected by selected arcs
        digraph<NT,AT>::const_arc_iterator output_arc = output(current, f);
        digraph<NT,AT>::const_iterator successor = arc_to(output_arc);
        if (!select || select(*this,output_arc))
        {
          ...
          {
            --fanin_map[successor];
            //IF_DEBUG(ferr << "decrementing successor node "; print_node(ferr,successor); ferr << " to " << fanin_map[successor] << endl); 
            if ((fanin_map[successor]) == 0)
            {
              result.push_back(successor);
              //IF_DEBUG(ferr << "added node "; print_node(ferr,successor); ferr << " to result" << endl); 
              fanin_map.erase(fanin_map.find(successor));
              //IF_DEBUG(ferr << "removed node "; print_node(ferr,successor); ferr << " from candidates" << endl); 
            }
          }
        }
      }
    ...
    if (!fanin_map.empty())
    {
      ...
      digraph<NT,AT>::const_iterator stuck_node = fanin_map.begin()->first;
      //IF_DEBUG(ferr << "trying to unstick node "; print_node(ferr,stuck_node); ferr << endl); 
      for (unsigned f = 0; f < fanin(stuck_node); f++)
      {
        ...
        digraph<NT,AT>::const_arc_iterator input_arc = input(stuck_node, f);
        if (!select || select(*this,input_arc))
        {
          digraph<NT,AT>::const_iterator predecessor = arc_from(input_arc);
          if (fanin_map.find(predecessor) != fanin_map.end())
          {
            //IF_DEBUG(ferr << "unsticking arc to predecessor "; print_node(ferr,predecessor); ferr << endl); 
            ...
            //IF_DEBUG(ferr << "decrementing stuck node "; print_node(ferr,stuck_node); ferr << " to " << fanin_map[stuck_node] << endl); 
            if ((fanin_map[stuck_node]) == 0)
            {
              result.push_back(stuck_node);
              //IF_DEBUG(ferr << "added node "; print_node(ferr,stuck_node); ferr << " to result" << endl); 
              fanin_map.erase(fanin_map.find(stuck_node));
              //IF_DEBUG(ferr << "removed node "; print_node(ferr,stuck_node); ferr << " from candidates" << endl); 
              break;
            }
          }
        }
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  ...
  for (unsigned i = 0; i < fanout(from); i++)
  {
    digraph<NT,AT>::const_arc_iterator arc = output(from,i);
    if (!select || select(*this, arc))
    {
      digraph<NT,AT>::const_iterator node = arc_to(arc);
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  ...
  digraph<NT,AT>::const_iterator_set visited;
  visited.insert(from);
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  ...
  for (unsigned i = 0; i < fanout(from); i++)
  {
    digraph<NT,AT>::const_arc_iterator candidate = output(from,i);
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  ...
  digraph<NT,AT>::const_path_vector result;
  digraph<NT,AT>::const_arc_vector so_far;
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  ...
  for (unsigned i = 0; i < fanout(from); i++)
  {
    ...
    if (!select || select(*this,arc))
    {
      digraph<NT,AT>::const_iterator candidate = arc_to(arc);
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  ...
  for (unsigned i = 0; i < fanin(to); i++)
  {
    digraph<NT,AT>::const_arc_iterator arc = input(to,i);
    if (!select || select(*this,arc))
    {
      digraph<NT,AT>::const_iterator candidate = arc_from(input(to,i));
      if (visited.insert(candidate).second)
...
typename digraph<NT,AT>::const_arc_vector digraph<NT,AT>::shortest_path(PARAMETER_TYPENAME digraph<NT,AT>::const_iterator from,
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  digraph<NT,AT>::const_path_vector paths = all_paths(from,to,select);
  digraph<NT,AT>::const_arc_vector shortest;
  ...
  throw(wrong_object,null_dereference,end_dereference)
{
  ...
  std::deque<digraph<NT,AT>::const_iterator> nodes;
  ...
  typedef std::map<digraph<NT,AT>::const_iterator,digraph<NT,AT>::const_arc_iterator> known_map;
  known_map known;
  known.insert(make_pair(from,digraph<NT,AT>::const_arc_iterator()));
  ...
  for (typename known_map::iterator i = known.begin(); i != known.end(); i++)
  {
    if (i->first != from)
    {
      ...
      for (typename known_map::iterator node = i; node->second != digraph<NT,AT>::const_arc_iterator(); node = known.find(arc_from(node->second)))
...
otext& digraph<NT,AT>::print(otext& str, unsigned indent) const
{
  ...
  print_indent(str,indent);
  str << "arcs:" << endl;
  for (digraph<NT,AT>::const_arc_iterator a = arc_begin(); a != arc_end(); ++a)
  {
    print_indent(str,indent+1);
    print_arc(str, a);
    str << " from ";
...
template<typename NT, typename AT>
otext& print_graph(otext& str, const PARAMETER_TYPENAME digraph<NT,AT>& graph, unsigned indent)
{
  return graph.print(str,indent);
}

template<typename NT, typename AT>
otext& operator << (otext& str, const PARAMETER_TYPENAME digraph<NT,AT>& graph)
{
  ...
  throw(persistent_restore_failed)
{
  ...
  for (unsigned n = 0; n < nodes; n++)
  {
    ...
    digraph<NT,AT>::iterator node = insert(NT());
  ...
  for (unsigned a = 0; a < arcs; a++)
  {
    ...
    digraph<NT,AT>::arc_iterator arc = arc_insert(digraph<NT,AT>::iterator(this,from), digraph<NT,AT>::iterator(this,to));

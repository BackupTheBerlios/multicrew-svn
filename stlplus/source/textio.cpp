...
otext::otext (const otext& source)
{
...
{
  // performance issue - repeatedly dereferencing a smart pointer can have a significant hit so dereference once
  obuff* buffer = m_buffer.pointer();
  ...
  else if (ch != newline)
  {
    ...
    if (buffer->put((unsigned char)ch))
      buffer->increment();
    else
    {
      set_error(textio_put_failed);
...
itext::itext (const itext& source)
{
  m_buffer = source.m_buffer;
}
...
int itext::peek (void)
{
  ibuff* buffer = m_buffer.pointer();
  if (!buffer) return -1;
  int ch = buffer->peek();
  if (buffer->newline_mode() == itext::convert_mode)
  {
    switch(ch)
    {
